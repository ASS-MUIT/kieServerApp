...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "kieserverapp")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name kieserverapp apps/kieserverapp:0.0.1-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app kieserverapp:0.0.1-SNAPSHOT
    oc expose svc/kieserverapp
else

	echo "Launching the application locally..."
	pattern="kieserverapp"
	files=( $pattern )
	cd ${files[0]}
